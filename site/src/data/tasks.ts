...
function singleTask(
  ...
  return {
    ...
    theorySlug,
    goal,
    description: details.description ?? "Прочитайте условие, начните с заготовки на странице задачи и напишите решение вместо TODO. Сначала добейтесь компиляции, затем проверьте поведение на простых данных.",
    practicePath,
    whatToCreate: details.whatToCreate ?? [`Один файл: ${practicePath}`],
    files: [
      {
        fileName: practicePath,
        description: details.files?.[0]?.description ?? "Стартовая заготовка задачи. Найдите TODO и допишите решение.",
...
export const tasks: Task[] = [
  ...
  singleTask("08-01-plate-constructors", "Plate: два конструктора", "08. Список инициализации", "medium", "initializer-list", ["initializer list", "constructor", "Plate"], "practice/08_initializer_list/ex01_plate_constructors.cpp", "Создать `Plate` с конструктором по умолчанию и полным конструктором через список инициализации.", {
    description: "Нужно описать класс `Plate` с закрытыми полями ширины, высоты и толщины. Оба конструктора должны задавать поля сразу после двоеточия, без присваивания в теле конструктора.",
    whatToCreate: ["класс `Plate`", "private-поля `width`, `height`, `thickness`", "конструктор по умолчанию", "полный конструктор `Plate(int width, int height, int thickness)`", "метод `print() const`", "проверку двух объектов в `main()`"],
    todoGuide: ["Объявите класс `Plate`.", "Добавьте поля `width`, `height`, `thickness`.", "Напишите конструктор по умолчанию: `Plate() : width(0), height(0), thickness(0) {}`.", "Напишите полный конструктор со списком `: width(width), height(height), thickness(thickness)`.", "Добавьте `print() const` для вывода размеров.", "В `main()` создайте пустую плитку и плитку с размерами.", "Проверьте вывод."],
    steps: ["Создайте поля класса.", "Добавьте конструктор по умолчанию.", "Добавьте полный конструктор.", "Добавьте метод вывода.", "Создайте два объекта и выведите их."],
    hints: ["Двоеточие ставится после параметров конструктора.", "В `width(width)` левый `width` — поле, правый `width` — параметр.", "Пишите поля в списке в том же порядке, что и в классе.", "Тело конструктора может быть пустым."],
    commonMistakes: ["Пишут `width = width` в теле конструктора.", "Путают поле и параметр с одинаковым именем.", "Ставят двоеточие внутри тела конструктора.", "Думают, что порядок в списке меняет порядок инициализации.", "Забывают точку с запятой после класса."],
    selfCheck: ["Оба конструктора используют список инициализации.", "В теле конструкторов нет присваивания полям.", "Поля перечислены в порядке объявления.", "`print() const` выводит размеры плитки.", "В `main()` проверены оба конструктора."],
    files: [{
      fileName: "practice/08_initializer_list/ex01_plate_constructors.cpp",
      description: "Один файл с классом Plate и проверкой двух конструкторов.",
...
`,
  ...
  singleTask("08-02-plate-store", "Plate store: список плиток", "08. Список инициализации", "medium", "initializer-list", ["initializer list", "vector", "menu", "Plate"], "practice/08_initializer_list/ex02_plate_store.cpp", "Сделать список заранее созданных плиток с добавлением, удалением и выводом.", {
    description: "Нужно использовать класс `Plate` из темы и хранить несколько плиток в `vector<Plate>`. Программа должна заранее добавить плитки, показать список, добавить новую плитку и удалить плитку по номеру.",
    whatToCreate: ["класс `Plate`", "конструктор `Plate` через список инициализации", "`vector<Plate> plates`", "несколько заранее созданных плиток", "функцию или пункт меню для вывода списка", "добавление новой плитки", "удаление плитки по номеру с проверкой"],
    todoGuide: ["Опишите `Plate` с полями `width`, `height`, `thickness`.", "Добавьте полный конструктор через список инициализации.", "Добавьте `print() const`.", "Создайте `vector<Plate> plates`.", "Добавьте 3 плитки заранее через `push_back`.", "Сделайте вывод списка с номерами от 1.", "Добавьте пункт создания новой плитки по введённым размерам.", "Перед удалением проверьте, что номер от 1 до `plates.size()`.", "Удаляйте через `plates.erase(plates.begin() + number - 1)`."],
    steps: ["Соберите класс `Plate`.", "Создайте `vector<Plate>`.", "Добавьте готовые плитки.", "Сделайте вывод списка.", "Добавьте ввод новой плитки.", "Добавьте безопасное удаление по номеру."],
    hints: ["Для `vector` нужен `#include <vector>`.", "Если пользователь вводит номер с 1, индекс равен `number - 1`.", "Перед `erase` обязательно проверьте границы.", "Сначала проверьте вывод заранее созданных плиток, потом добавляйте меню."],
    commonMistakes: ["Инициализируют поля присваиванием в теле конструктора вместо списка.", "Удаляют элемент без проверки номера.", "Путают номер пользователя и индекс `vector`.", "Забывают вывести обновлённый список после добавления или удаления.", "Пытаются использовать список инициализации в обычной функции добавления."],
    selfCheck: ["В списке есть заранее созданные плитки.", "Новая плитка добавляется в конец списка.", "Плитка удаляется по корректному номеру.", "Неверный номер не ломает программу.", "Конструктор `Plate` использует список инициализации."],
    files: [{
      fileName: "practice/08_initializer_list/ex02_plate_store.cpp",
      description: "Один файл со списком плиток и простым меню.",
      starterCode: `#include <iostream>
#include <vector>
...
int main() {
    ...
    // TODO: добавьте 3 заранее созданные плитки.
    // TODO: сделайте меню вывода, добавления и удаления.
...
` }],
  ...
  singleTask("task5-1-plate", "Задача 5.1: класс Plate", "12. Большие задачи", "hard", "initializer-list", ["initializer list", "vector", "menu"], "practice/big_tasks/task5_1_plate.cpp", "Создать класс `Plate` со списком инициализации, заранее заданными объектами и меню.", {
    description: "Задача соединяет список инициализации конструктора и простой список объектов. Нужно создать класс плитки, заранее добавить несколько плиток и сделать меню добавления, вывода и удаления.",
    whatToCreate: ["класс `Plate`", "private-поля `width`, `height`, `thickness`, `material`, `price`", "конструктор по умолчанию", "полный конструктор через список инициализации", "`vector<Plate> plates`", "3-5 заранее созданных плиток", "меню вывода, добавления и удаления"],
    todoGuide: ["Объявите класс `Plate`.", "Добавьте private-поля размеров, материала и цены.", "Напишите конструктор по умолчанию через список инициализации.", "Напишите полный конструктор через список инициализации.", "Добавьте метод `print() const`.", "В `main` создайте `vector<Plate> plates`.", "Добавьте 3-5 готовых плиток через `push_back`.", "Сделайте меню: показать список, добавить плитку, удалить плитку, выйти.", "При удалении проверяйте номер от 1 до `plates.size()`."],
    steps: ["Опишите private-поля.", "Напишите конструктор по умолчанию через список инициализации.", "Напишите полный конструктор через список инициализации.", "Добавьте метод `print() const`.", "Создайте `vector<Plate>` и заполните 3-5 объектами.", "Сделайте вывод списка.", "Добавьте добавление и удаление с проверкой номера."],
    hints: ["Список инициализации начинается после `:`.", "Поля в списке разделяются запятыми.", "В `width(width)` левый `width` — поле, правый `width` — параметр.", "Порядок в списке лучше держать таким же, как порядок объявления полей.", "Создайте объект `Plate plate(...);` и добавьте через `plates.push_back(plate)`.", "Удаление делайте через `plates.erase(plates.begin() + number - 1)` только после проверки."],
    commonMistakes: ["Пишут присваивание в теле конструктора вместо списка инициализации.", "Путают параметр и поле с одинаковым именем.", "Думают, что порядок в списке управляет порядком инициализации.", "Ставят двоеточие в обычном методе, а не в конструкторе.", "Пытаются использовать список инициализации внутри функции добавления.", "Удаляют плитку без проверки номера."],
    selfCheck: ["Оба конструктора используют список инициализации.", "Поля в списке идут в порядке объявления.", "В списке есть заранее созданные плитки.", "Можно добавить новую плитку.", "Можно удалить плитку по корректному номеру.", "Неверный номер удаления не ломает программу.", "После изменения список выводится актуально."],
