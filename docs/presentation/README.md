...
- Vite 8 build tooling и route-level lazy split задеплоены.
- ООП C++ разделы 0-10 закрыты без пробела; разделы 11-12 остаются content backlog.
- Checker data foundation и draft flow реализованы локально, но runner и запуск пользовательского кода отсутствуют; это не production-функция до review/merge/deploy.
...
8. Завершить планом развития: UI audit, review checker draft flow, безопасный deploy, отдельный runner, затем learning loop и sections 11-12.
...
- спроектировать isolated runner и включить проверку на простых задачах;
- улучшить learning loop, затем завершить sections 11-12 и audit 0-12;
- teacher/classroom и monetization рассматривать только после отдельных product/security reviews.
